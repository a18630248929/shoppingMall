...
App({
  onLaunch: function() {
    ...
    wx.login({
      success: res => {
        this.globalData.mcode = res.code
        // console.log(res)
        wx.request({ 
          url: 'https://preapi.baiwangkeji.com/pingpp/SmallOpenID', //开发者服务器接口地址",
          data: {
            code: res.code,
            platform: 'shop'
          }, //请求的参数",
    ...
    if (this.globalData.susTopicCompids && this.globalData.susTopicCompids.length) { // 有悬浮窗话题列表判断是否要显示向上按钮
      ...
      for (let i in (this.globalData.susTopicCompids)) {
        ...
        if ((pageInstance.data[compid].topicSuspension.topBtnShow || false) !== topBtnShow) {
          pageInstance.setData({
            [compid + '.topicSuspension.topBtnShow']: topBtnShow
          });
        }
      }
    }
  ...
  refreshOneTopicData: function(param) {
    ...
    this.sendRequest({
      ...
      success: function(res) {
        ...
        pageInstance.setData({
          [param.compid + '.topicList[' + param.index + ']']: res.data[0]
        });
      },
      fail: function() {}
    });
  ...
  showTopicCommentBox: function(e) {
    ...
    pageInstance.setData({
      [compid + '.topicList[' + index + '].comment_box_show']: !commentBoxShow
    });
  },
  showTopicPhoneModal: function(e) {
    ...
    pageInstance.setData({
      [compid + '.topicPhoneModal']: topicPhoneModal
    });
  },
  topicMakePhoneCall: function(e) {
    ...
    pageInstance.setData({
      [compid + '.topicPhoneModal.isShow']: false
    });
  ...
  topicCommentReplyInput: function(e) {
    let pageInstance = this.getAppCurrentPage(),
      compid = e.currentTarget.dataset.compid;
    pageInstance.setData({
      [compid + '.topicReplyComment.text']: e.detail.value
    });
  },
      ...
      success: function(res) {
        if (res.status == 0) {
          that.showToast({
            ...
            success: function() {
              pageInstance.setData({
                [compid + '.topicList[' + index + '].comment_box_show']: false
              });
    ...
    if (pageInstance.signInComps.length) {
      for (let i = 0; i < pageInstance.signInComps.length; i++) {
        ...
        if (!!pageInstance.data[compid].activity_id) {
          customComponent['sign-in'].signInExtraRewards(compid, pageInstance);
          customComponent['sign-in'].signInGetUserNotice(compid, pageInstance);
          customComponent['sign-in'].signInRewardCheck(compid, pageInstance);
        }
      }
    }
  },
  ...
  returnListHeight: function(isshow, takeout) {
    if (!isshow) {
  ...
  pageScrollFunc: function(event) {
    ...
    if (compData.is_search) {
      customComponent["serch"].searchList(compData.searchEle, compData.compId, event);
    } else {
  ...
  turnToGoodsDetail: function(event) {
    ...
    switch (+goodsType) {
      case 0:
        this.turnToPage('/pages/goodsDetail/goodsDetail?detail=' + id + '&contact=' + contact + '&hidestock=' + hidestock + '&isShowVirtualPrice=' + isShowVirtualPrice + chainParam);
        break;
      case 1:
        this.turnToPage('/pages/goodsDetail/goodsDetail?detail=' + id + '&contact=' + contact + '&hidestock=' + hidestock + chainParam);
        break;
      case 3:
  ...
  sortListFunc: function(event) {
    ...
    switch (listType) {
      ...
      case 'list-vessel':
        this._sortListVessel(new_component_params, dataset);
        break;
      case 'group-buy-list':
        customComponent["group-buy-list"].getGroupBuyList(new_component_params.compid, new_component_params, dataset);
        break;
      case 'goods-list':
        this._sortGoodsList(new_component_params, dataset);
        break;
      case 'franchisee-list':
        this._sortFranchiseeList(new_component_params, dataset);
        break;
      case 'video-list':
        this._sortVideoList(new_component_params, dataset);
        break;
    }
  },
  ...
  _refreshPageList: function(eleType, eventParams, compids_params, pageInstance) {
    ...
    switch (eleType) {
      case 'goods-list':
        this._refreshGoodsList(compids_params['compid'], requestData, pageInstance);
        break;
      case 'list-vessel':
        this._refreshListVessel(compids_params['compid'], requestData, pageInstance);
        break;
      case 'franchisee-list':
        this._refreshFranchiseeList(compids_params['compid'], requestData, pageInstance);
        break;
      case 'topic-list':
        this._refreshTopicList(compids_params['compid'], requestData, pageInstance);
        break;
      case 'news-list':
        this._refreshNewsList(compids_params['compid'], requestData, pageInstance);
        break;
      case 'video-list':
        this._refreshVideoList(compids_params['compid'], requestData, pageInstance);
        break;
      case 'group-buy-list':
        this._refreshGroupBuyList(compids_params['compid'], requestData, pageInstance);
        break;
    }
  },
  ...
  _refreshTopicList: function(targetCompId, requestData, pageInstance) {
    ...
    customComponent['topic'].getTopListData(pageInstance, {
      page: 1,
  ...
  _refreshNewsList: function(targetCompId, requestData, pageInstance) {
    ...
    customComponent['news'].getNewsList({
      page: 1,
      compid: targetCompId,
      category_id: requestData.idx_arr.idx_value
    });
  },
  ...
  sureTakeNumber: function(e) {
    ...
    if (pageInstance.data[compid].isClick) {
      return
    }
    ...
    switch (tapType) {
      case 'heightSus':
        if (compData.customFeature.pickUpType == 1) {
          res.animation.height(h + 'rpx').step({
            duration: 600
          });
        } else {
          res.animation.width(w + 'rpx').step({
            duration: 600
          });
        }
      ...
      case 'foldSus':
        if (compData.customFeature.pickUpType == 1) {
          res.animation.height(0).step({
            duration: 600
          });
        } else {
          res.animation.width(0).step({
            duration: 600
          });
        }
        newData[compId + '.customFeature.animations.unfoldSus'] = res.animation.export();
  ...
  globalData: {
    ...
    userDomain: 'https://u2228395.jisuwebapp.com', // 用户子域名
    appTitle: '我的应用9',
    appDescription: '我的应用',
    ...
    cart_id: '', //购物车勾选的id
    currtab: 0, //储存全部订单下标
    direct: 0, //跳转确认订单页存储direct 1立即购买 0购物车购买
    mcode: '',
    netease: []  //网易数据
  },
})
var componentArr = ["carousel", "group-buy-list"];
var customComponent = {}
